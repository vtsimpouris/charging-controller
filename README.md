...
The branches start at the open-loop poles at `s = 0` and `s = -2`. As `Kp` increases, the poles move toward `s = -1`, meet there at `Kp = 1`, and then form a complex-conjugate pair along `Re(s) = -1`. The selected value `Kp = 2` places the poles at `s = -1 ± j`, giving a stable, moderately damped response with approximately **4.3% theoretical overshoot**.
